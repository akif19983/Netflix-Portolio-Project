...
-- To categorised Movie for each country

With CountryMovie(Country, type, TotalMovie)
as
(
...
-- To categorised TV Show for each country

With CountryTVShow(Country, type, TotalTVShow)
as
(
Select country, type, COUNT(Country) as TotalTVShow
From NetflixPortfolioProject..Netflix
Where country is not null 
and country <> 'NotGiven'
Group by country, type
...
Select release_year, count(show_id) as TotalMovieRelease
From NetflixPortfolioProject..Netflix
...
Select release_year, count(show_id) as TotalTVShowRelease
From NetflixPortfolioProject..Netflix
Where release_year >= 2000
	and type = 'TV Show'
Group by release_year
